...
- **稳定直达链接**：在详情页点击 `Copy Link` 可复制 `?share=<shareKey>` 形式的本机 permalink，刷新或重扫后仍会尽量回到同一逻辑文档

服务仅绑定 `127.0.0.1`（本地访问），不暴露到局域网。

#### 稳定直达链接

在预览页打开任意 session 或 aggregate 文档后，可以使用详情头部的 `Copy Link` 按钮复制当前文档的本机直达链接，例如：

```text
http://127.0.0.1:3000/?share=workspace%3Asession%3A2026-03-18-demo-session
```

说明：

- 链接仅用于当前机器上的 `serve` 预览，不是跨机器分享链接
- 链接依赖当前扫描 root 下仍然存在同一逻辑文档
- 当前只支持文件级直达，不支持单条 memory 深链
