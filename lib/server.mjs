...
const SERVE_CONTENT_HIERARCHY_MODULE = path.resolve(
  __dirname,
  "serve-content-hierarchy.mjs",
);
...
function handleServeContentHierarchyModule(res) {
  let source;
  try {
    source = fs.readFileSync(SERVE_CONTENT_HIERARCHY_MODULE, "utf-8");
  } catch (err) {
    jsonResponse(res, 500, {
      error: `Cannot read serve-content-hierarchy module: ${err.message}`,
    });
    return;
  }
...
function createRequestHandler(rootPath) {
  return (req, res) => {
    ...
    // GET /assets/serve-content-hierarchy.mjs
    if (
      (method === "GET" || method === "HEAD") &&
      pathname === "/assets/serve-content-hierarchy.mjs"
    ) {
      handleServeContentHierarchyModule(res);
      return;
    }
