...
export function runSetup(mode) {
  ...
  // Step 2.2: Copy recall hook runner and helpers
  log.info("Installing recall hook runtime...");
  const recallRunnerSrc = path.join(TEMPLATES_DIR, "hooks", "cursor-memory-recall.mjs");
  const recallRunnerDest = path.join(targetDir, "hooks", "cursor-memory-recall.mjs");
  copyFile(recallRunnerSrc, recallRunnerDest);
  const recallContractSrc = path.join(
    TEMPLATES_DIR,
    "hooks",
    "cursor-memory-recall-contract.mjs",
  );
  const recallContractDest = path.join(
    targetDir,
    "hooks",
    "cursor-memory-recall-contract.mjs",
  );
  copyFile(recallContractSrc, recallContractDest);
  const recallRuntimeSrc = path.join(
    TEMPLATES_DIR,
    "hooks",
    "cursor-memory-recall-runtime.mjs",
  );
  const recallRuntimeDest = path.join(
    targetDir,
    "hooks",
    "cursor-memory-recall-runtime.mjs",
  );
  copyFile(recallRuntimeSrc, recallRuntimeDest);
  log.success(`Recall runtime installed at ${recallRunnerDest}`);

  // Step 2.3: Install default archive config if missing
  ...
  log.dim(`  - hooks/cursor-memory-recall.mjs`);
