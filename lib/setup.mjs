...
import { TEMPLATES_DIR, resolveTargetDir, HOOK_SCRIPT_NAME } from './constants.mjs';
import { readHooksConfig, mergeHooksConfig, writeHooksConfig } from './hooks.mjs';
...
export function runSetup(mode) {
  ...
  const modeLabel = mode === 'global' ? 'global (~/.cursor/)' : `local (${targetDir}/)`;
  ...
  log.info('Installing catch-memories command...');
  const cmdSrc = path.join(TEMPLATES_DIR, 'commands', 'catch-memories.md');
  const cmdDest = path.join(targetDir, 'commands', 'catch-memories.md');
  ...
  log.dim(`  - commands/catch-memories.md`);
}
