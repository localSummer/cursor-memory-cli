...
function timestampFromSessionFile(dateDir, fileName) {
  ...
  const timePart = base.slice(0, 8); // "HH-MM-SS"
  return `${dateDir}T${timePart.replace(/-/g, ":")}`;
}
...
function scanDateDirs(memoriesDir, fileIndex, projectId) {
  ...
  for (const entry of entries) {
    ...
    for (const fileName of files) {
      ...
      fileObjs.push({ id, name: fileName, timestamp, kind: "session" });
      fileIndex.set(id, {
...
function scanAggregateFiles(aggregateDir, fileIndex, projectId) {
  ...
  for (const fileName of files) {
    ...
    results.push({ id, name: fileName, month, kind: "aggregate" });
    fileIndex.set(id, {
...
export function readMemoryFile(rootPath, id, fileIndex) {
  ...
  const { absPath, kind: indexKind } = entry;
